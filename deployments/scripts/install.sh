...
LATEST_RC=$(git ls-remote --exit-code --sort='v:refname' --tags https://github.com/mosajjal/horusec.git --ref 'v*.*.*-rc.*' | cut --delimiter='/' --fields=3 | tail --lines=1 | sed 's/.*\///; s/\^{}//')
LATEST_BETA=$(git ls-remote --exit-code --sort='v:refname' --tags https://github.com/mosajjal/horusec.git --ref 'v*.*.*-beta.*' | cut --delimiter='/' --fields=3 | tail --lines=1 | sed 's/.*\///; s/\^{}//')
IS_NEW_URL=false
...
horusecSetVersion() {
  if [ -z "$VERSION_DOWNLOAD" ]; then
    ...
    echo "input not match required params: 'latest-rc' 'latest-beta' 'latest' or a semantic version compliant, check https://github.com/mosajjal/horusec/releases"
    exit 1
  fi
}
...
horusecIdentifyOSLatest() {
  if [ "$(uname)" = "Linux" ]; then
    if [ "$(uname -m)" = "x86_64" ]; then
      ...
      URL_DOWNLOAD="https://github.com/mosajjal/horusec/releases/${VERSION_DOWNLOAD}/download/horusec_linux_amd64"
    elif [ "$(uname -m)" = "aarch64" ]; then
      echo "Installing Horusec for Linux arm64"
      URL_DOWNLOAD="https://github.com/mosajjal/horusec/releases/${VERSION_DOWNLOAD}/download/horusec_linux_arm64"
    else
      echo "Installing Horusec for Linux x86"
      URL_DOWNLOAD="https://github.com/mosajjal/horusec/releases/${VERSION_DOWNLOAD}/download/horusec_linux_x86"
    fi
  elif [ "$(uname)" = "Darwin" ]; then
    if [ "$(uname -m)" = "x86_64" ]; then
      echo "Installing Horusec for Mac amd64"
      URL_DOWNLOAD="https://github.com/mosajjal/horusec/releases/${VERSION_DOWNLOAD}/download/horusec_mac_amd64"
    elif [ "$(uname -m)" = "arm64" ]; then
      echo "Installing Horusec for Mac arm64"
      URL_DOWNLOAD="https://github.com/mosajjal/horusec/releases/${VERSION_DOWNLOAD}/download/horusec_mac_arm64"
    else
  ...
  if [ "$(uname)" = "Linux" ]; then
    if [ "$(uname -m)" = "x86_64" ]; then
      ...
      if [ $IS_NEW_URL = true ]; then
        URL_DOWNLOAD="https://github.com/mosajjal/horusec/releases/download/${VERSION_DOWNLOAD}/horusec_linux_amd64"
      else
        URL_DOWNLOAD="https://github.com/mosajjal/horusec/releases/download/${VERSION_DOWNLOAD}/horusec_linux_x64"
      fi
    elif [ "$(uname -m)" = "aarch64" ]; then
      echo "Installing Horusec for Linux arm64"
      URL_DOWNLOAD="https://github.com/mosajjal/horusec/releases/download/${VERSION_DOWNLOAD}/horusec_linux_arm64"
    else
      echo "Installing Horusec for Linux x86"
      URL_DOWNLOAD="https://github.com/mosajjal/horusec/releases/download/${VERSION_DOWNLOAD}/horusec_linux_x86"
    fi
  elif [ "$(uname)" = "Darwin" ]; then
    if [ "$(uname -m)" = "x86_64" ]; then
      echo "Installing Horusec for Mac amd64"
      if [ $IS_NEW_URL = true ]; then
        URL_DOWNLOAD="https://github.com/mosajjal/horusec/releases/download/${VERSION_DOWNLOAD}/horusec_mac_amd64"
      else
        URL_DOWNLOAD="https://github.com/mosajjal/horusec/releases/download/${VERSION_DOWNLOAD}/horusec_mac_x64"
      fi
    elif [ "$(uname -m)" = "x86_64" ]; then
      echo "Installing Horusec for amr64"
      URL_DOWNLOAD="https://github.com/mosajjal/horusec/releases/download/${VERSION_DOWNLOAD}/horusec_mac_arm64"
    else
      echo "Not enable Horusec to Mac x86"
      exit 1
    fi
